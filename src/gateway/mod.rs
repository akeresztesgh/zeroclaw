...
use crate::cost::{shared_cost_tracker, shared_tool_usage_tracker, CostTracker, ToolUsageTracker};
...
pub struct AppState {
    ...
    /// Tool usage tracker (optional, for web dashboard cost page)
    pub tool_usage_tracker: Option<Arc<ToolUsageTracker>>,
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let cost_tracker = if config.cost.enabled {
        match shared_cost_tracker(config.cost.clone(), &config.workspace_dir) {
            Ok(ct) => Some(ct),
            Err(e) => {
    ...
    let tool_usage_tracker = if config.cost.enabled {
        match shared_tool_usage_tracker(config.cost.clone(), &config.workspace_dir) {
            Ok(tt) => Some(tt),
            Err(e) => {
    ...
    // Wrap observer with cost tracking + broadcast capability for SSE
    let mut observers: Vec<Box<dyn crate::observability::Observer>> =
        vec![crate::observability::create_observer(&config.observability)];
    ...
    let base_observer: Box<dyn crate::observability::Observer> = if observers.len() == 1 {
        observers.remove(0)
    } else {
        Box::new(crate::observability::MultiObserver::new(observers))
    };
