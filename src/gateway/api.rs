...
pub async fn handle_api_cost(
    ...
    if let Some(ref tracker) = state.cost_tracker {
        match tracker.get_summary() {
            Ok(mut summary) => {
                if let Some(ref tool_tracker) = state.tool_usage_tracker {
                    if let Ok(tool_summary) = tool_tracker.get_summary() {
                        summary.tool_calls_session = tool_summary.session_total;
                        summary.tool_calls_daily = tool_summary.daily_total;
                        summary.tool_calls_monthly = tool_summary.monthly_total;
                        summary.tool_calls_by_tool = tool_summary.by_tool;
                    }
                }
                let (workspace_dir, cost_path, cost_exists, cost_len) = {
                    let cfg_guard = state.config.lock();
                    let path = cfg_guard.workspace_dir.join("state").join("costs.jsonl");
                    let meta = std::fs::metadata(&path).ok();
                    (
                        cfg_guard.workspace_dir.display().to_string(),
