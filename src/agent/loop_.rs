...
pub async fn run(
    ...
    tool_descs.push((
        "cron_add",
        "Create a cron job. Supports schedule kinds: cron, at, every; and job types: shell or agent. For channel reminders with delivery.mode=announce, set delivery.channel to the current channel and delivery.to to the current reply_target exactly.",
    ));
    tool_descs.push((
        "cron_list",
