    ...
    match channel_name {
        "telegram" => Some(
            "When responding on Telegram:\n\
             ...
             - For scheduled responses via `cron_add` with `delivery.mode=\"announce\"`, `delivery.to` must exactly match the current inbound `reply_target` (chat_id or chat_id:thread_id). Do not synthesize IDs from sender names or memory keys.",
        ),
        _ => None,
    }
}
...
async fn process_channel_message(
    ...
    let system_prompt = format!(
        "{base_system_prompt}\n\n## Current Routing Context\n\ncurrent_channel: {}\ncurrent_reply_target: {}\nFor `cron_add` with `delivery.mode=\"announce\"`, set `delivery.channel` to `current_channel` and set `delivery.to` to `current_reply_target` exactly. Never derive delivery targets from sender IDs, usernames, conversation keys, or memory keys.",
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let cost_tracker = if config.cost.enabled {
        match crate::cost::shared_cost_tracker(config.cost.clone(), &config.workspace_dir) {
            Ok(ct) => Some(ct),
            Err(e) => {
    ...
    let tool_usage_tracker = if config.cost.enabled {
        match crate::cost::shared_tool_usage_tracker(config.cost.clone(), &config.workspace_dir) {
            Ok(tt) => Some(tt),
            Err(e) => {
                tracing::warn!("Failed to initialize tool usage tracker: {e}");
                None
            }
        }
    } else {
    ...
    let mut observers: Vec<Box<dyn Observer>> =
        vec![observability::create_observer(&config.observability)];
    if let Some(ct) = &cost_tracker {
        observers.push(Box::new(crate::observability::CostObserver::new(
            Arc::clone(ct),
            tool_usage_tracker.clone(),
            config.cost.clone(),
        )));
    }
    let observer: Arc<dyn Observer> = if observers.len() == 1 {
        Arc::from(observers.remove(0))
    } else {
        Arc::from(observability::MultiObserver::new(observers))
    };
