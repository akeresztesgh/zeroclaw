...
pub struct SkillsConfig {
    ...
    /// Allow script-like files inside skill directories.
    /// Default: `false` (opt-in).
    #[serde(default)]
...
pub struct CostConfig {
    ...
    /// Fallback pricing (USD per 1M tokens) for unknown models
    #[serde(default = "default_fallback_pricing")]
    pub fallback_pricing: ModelPricing,

    /// Timezone for daily/monthly cost aggregation (IANA name, default: UTC)
...
impl Default for CostConfig {
    fn default() -> Self {
        Self {
            ...
            timezone: default_cost_timezone(),
        }
    }
}
