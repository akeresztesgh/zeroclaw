...
pub fn load_skills(workspace_dir: &Path) -> Vec<Skill> {
    load_skills_with_open_skills_config(workspace_dir, None, None, audit::SkillAuditPolicy::default())
}

/// Load skills using runtime config values (preferred at runtime).
pub fn load_skills_with_config(workspace_dir: &Path, config: &crate::config::Config) -> Vec<Skill> {
    let policy = audit::SkillAuditPolicy {
...
fn load_skills_with_open_skills_config(
    ...
    {
        skills.extend(load_open_skills(&open_skills_dir, &policy));
    }

    skills.extend(load_workspace_skills(workspace_dir, &policy));
    skills
}

fn load_workspace_skills(workspace_dir: &Path, policy: &audit::SkillAuditPolicy) -> Vec<Skill> {
    let skills_dir = workspace_dir.join("skills");
    load_skills_from_directory(&skills_dir, policy)
}

fn load_skills_from_directory(skills_dir: &Path, policy: &audit::SkillAuditPolicy) -> Vec<Skill> {
    ...
    for entry in entries.flatten() {
        ...
        match audit::audit_skill_directory(&path, &policy) {
...
fn load_open_skills(repo_dir: &Path, policy: &audit::SkillAuditPolicy) -> Vec<Skill> {
    ...
    if nested_skills_dir.is_dir() {
        return load_skills_from_directory(&nested_skills_dir, policy);
    }
...
fn enforce_skill_security_audit(
    ...
    let report = audit::audit_skill_directory(skill_path, policy)?;
    if report.is_clean() {
        return Ok(report);
    }
...
fn install_local_skill_source(
    ...
    let _ = enforce_skill_security_audit(&source_path, policy)?;
    ...
    match enforce_skill_security_audit(&dest, policy) {
...
fn install_git_skill_source(
    ...
    match enforce_skill_security_audit(&installed_dir, policy) {
...
pub fn handle_command(command: crate::SkillCommands, config: &crate::config::Config) -> Result<()> {
    ...
    match command {
        ...
        crate::SkillCommands::Audit { source } => {
            ...
            let report = audit::audit_skill_directory(&target, &policy)?;
            if report.is_clean() {
                println!(
        ...
        crate::SkillCommands::Install { source } => {
            ...
            if is_git_source(&source) {
                let (installed_dir, files_scanned) =
                    install_git_skill_source(&source, &skills_path, &policy)
                ...
                let (dest, files_scanned) =
                    install_local_skill_source(&source, &skills_path, &policy)
