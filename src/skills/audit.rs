...
pub struct SkillAuditPolicy {
    pub allow_scripts: bool,
}

impl Default for SkillAuditPolicy {
    fn default() -> Self {
        Self {
            allow_scripts: false,
        }
...
pub fn audit_skill_directory(
    ...
    for path in collect_paths_depth_first(&canonical_root)? {
        ...
        audit_path(&canonical_root, &path, policy, &mut report)?;
    }
...
fn audit_path(
    ...
    if !policy.allow_scripts && is_unsupported_script_file(path) {
        report.findings.push(format!(
...
mod tests {
    ...
    fn audit_accepts_safe_skill() {
        ...
        let report = audit_skill_directory(&skill_dir, &SkillAuditPolicy::default()).unwrap();
...
"#,
    ...
    fn audit_allows_existing_cross_skill_reference() {
        ...
        let report = audit_skill_directory(&skill_a, &SkillAuditPolicy::default()).unwrap();
        assert!(
            report
                .findings
                .iter()
